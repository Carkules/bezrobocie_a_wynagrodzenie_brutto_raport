...
data_mod |> filter(Płeć == "ogółem", Rok == 2024) |>
  ...
  labs(x = "Wynagrodzenie przeciętne",
       y = "Stopa bezrobocia")
```
Na powyższym wykresie możemy zaobserwować, że istnieją powiaty zarówno ze stosunkowo niskim bezrobociem i wynagrodzeniem, jak i powiaty z wysokim wynagrodzeniem i niskim bezrobociem. Większość danych tworzy „chmurę", która jest słabo dopasowana do linii regresji. Stąd wnioskujemy, że dane są słabo skorelowane. Możemy to potwierdzić obliczając współczynnik korelacji Pearsona dla każdego roku.
```{r}
data_mod |> filter(Płeć == "ogółem") |> group_by(Rok) |> summarize(r = cor(stopa_bezrobocia, Wynagrodzenie_przeciętne))
```
